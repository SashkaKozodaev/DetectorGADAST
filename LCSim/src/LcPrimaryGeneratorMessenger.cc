...
//
//
// $Id: LcPrimaryGeneratorMessenger.cc,v 1.3 2006/06/29 17:54:29 gunter Exp $
...
LcPrimaryGeneratorMessenger::LcPrimaryGeneratorMessenger(
                                          LcPrimaryGeneratorAction* LcGun)
:LcAction(LcGun)
{
  gunDir = new G4UIdirectory("/Lc/gun/");
  gunDir->SetGuidance("PrimaryGenerator control");
   
  polarCmd = new G4UIcmdWithADoubleAndUnit("/Lc/gun/optPhotonPolar",this);
...
void LcPrimaryGeneratorMessenger::SetNewValue(
                                        G4UIcommand* command, G4String newValue)
{ 
  if( command == polarCmd ) {
      G4double angle = polarCmd->GetNewDoubleValue(newValue);
      if ( angle == -360.0*deg ) {
         LcAction->SetOptPhotonPolar();
      } else {
         LcAction->SetOptPhotonPolar(angle);
      }
  }
}
